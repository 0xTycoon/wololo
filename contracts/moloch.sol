...
contract Moloch is ReentrancyGuard {
    ...
    *****************/
    ...
    function _submitProposal(
        ...
        emit SubmitProposal(applicant, sharesRequested, lootRequested, tributeOffered, tributeToken, paymentRequested, paymentToken, details, flags, totals.proposalCount, msg.sender, memberAddress);
    ...
    function submitVote(uint256 proposalIndex, uint8 uintVote) public nonReentrant onlyDelegate {
        ...
        if (vote == Vote.Yes) {
            ...
            if (totals.totalShares.add(totals.totalLoot) > proposal.maxTotalSharesAndLootAtYesVote) {
                proposal.maxTotalSharesAndLootAtYesVote = totals.totalShares.add(totals.totalLoot);
            }
    ...
    ***************/
    ...
    function fairShare(uint256 balance, uint256 shares, uint256 totalShares) internal view returns (uint256) {
        require(totals.totalShares != 0);
        ...
        if (prod / balance == shares) { // no overflow in multiplication above?
            return prod / totals.totalShares;
        }

        return (balance / totals.totalShares) * shares;
    }
}
